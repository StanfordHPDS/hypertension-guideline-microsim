...
##GENERATE THE HEALTH SYSTEM TRANSITIONS
def generate_transitions_HS_sff(
    current_state_HS,
    current_state_BP,
    insurance,
    age,
    ...
    if current_state_BP == "D":
        ...
        # if we are at a measurement visit
        if measurement_interval == 0:
            pDT = generate_treatment_prob_SDI(
        ...
        # otherwise, we are not at a measurement visit, decrease one month
        else:
            pDT = 0
        ...
        # assuming that they get on insurance at age 65
        if insurance == "N" and age < 65:
            pDTUT = convert_to_prob(convert_to_rate(pDTUT) * rrDTUT_no_ins)
...
out_of_system_SBP = pd.read_csv(
    f"{overall_folder}/data_and_models/nhanes_inputs/out_of_system_sys_quantile_betas.csv"
)
out_of_system_DBP = pd.read_csv(
    f"{overall_folder}/data_and_models/nhanes_inputs/out_of_system_dia_quantile_betas.csv"
)


def generate_next_BP_stage_sff(
    this_quantile,
    this_quantile_dbp,
    current_state_HS,
    time_treated,
    ...
    if current_state_BP != "D":
        new_SBP = generate_new_SBP_value_SDI(
            sb_quantile_models, systolic_splines, this_quantile, age, sex, SDI
        )
        new_DBP = generate_new_DBP_value_SDI(
            db_quantile_models, diastolic_splines, this_quantile_dbp, age, sex, SDI
        )

        if current_state_HS == "OHS":
            sys_place_val = out_of_system_SBP["noplace_beta"].iloc[this_quantile]
            new_SBP = new_SBP + sys_place_val

            dia_place_val = out_of_system_DBP["noplace_beta"].iloc[this_quantile_dbp]
            new_DBP = new_DBP + dia_place_val
...
def generate_transitions_DNH_sff(
    current_state_BP,
    current_DBP_val,
    current_SBP_val,
    age,
    ...
    # Get the mortality probability from the life table based on insurance status
    # don't know if I will implement the Medicare assumption yet
    if age < 65:
        if insurance == "Y":
            pHD_column = "qx_ins_cycle"
        else:
            pHD_column = "qx_no_ins_cycle"
    else:
        pHD_column = "qx_cycle"
